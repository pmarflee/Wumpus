...
type GameTestFixture () =
    ...
    let ``Player should be able to move to a different room if a valid exit is selected`` () =
        let game = new Game(initWithNoHazards, batRoomMoveCalculator, wumpusMoveCalculator)
    ...
    let ``Player should lose game if they enter a room containing a pit`` () =
        let game = new Game(initWithHazard 1 Hazard.Pit, batRoomMoveCalculator, wumpusMoveCalculator)
        game.MovePlayer(1)
    ...
    let ``Player and bat should be carried into another room if they enter a room containing a bat`` () =
        ...
        let game = new Game(initWithHazard 1 Hazard.Bat, batRoomMoveCalculator, wumpusMoveCalculator)
        game.MovePlayer(1)
